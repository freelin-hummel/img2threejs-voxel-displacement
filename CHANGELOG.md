...
- Publish the installer to npm from a release tag: `.github/workflows/npm-publish.yml` calls the reviewed `ci-workflows` publish workflow, which runs the test suite in a job with no access to the npm token, verifies the tag against both `SKILL.md` and `package.json`, refuses a manifest declaring an install lifecycle script, and publishes with `--provenance` under the tag's prerelease identifier (`latest` for a stable tag). `npx -y img2threejs@latest install` now works alongside the `github:` form.
- Bump every version-bearing file together on a beta release. `beta-release.yml` previously rewrote `SKILL.md` alone, leaving the README badge stale against an assertion that already checked it; it now rewrites the badge and `package.json` in the same commit and fails closed if any pattern stops matching.
